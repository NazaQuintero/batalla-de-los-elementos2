...
const int CANTIDAD_PERSONAJES_X_JUGADOR = 3;
...
int Juego::ejecutarOpcionSecundario(int opcion, Diccionario<string, Personaje*> *diccionarioDePersonajes) {
	int systemResult = 0;
		switch (opcion) {
			...
			case 3:
				systemResult = this->utilitario->limpiarPantalla();
				for (int i = 0; i < CANTIDAD_PERSONAJES_X_JUGADOR; i++) {
    ...
    *costoEnergia = 0;
    ...
    if(!casilleroNuevo->getPersonaje()){
        ...
        if(personaje->obtenerEnergia() >= *costoEnergia){
            ...
            sleep(2);
            personaje->setPosicion(posX, posY);
            personaje->bajarEnergia(*costoEnergia);
            casilleroNuevo->setPersonaje(personaje);
        } else {
        	cout << "El personaje no tiene energia suficiente." << endl;
        	sleep(4);
        }
    }
}
...
void Juego::obtenerCamino(Personaje* personaje, Lista<Casillero*>* pilaCamino, int* posFinal, int* costoEnergia) {
    ...
    if(posInicial[0] == posFinal[0] && posInicial[1] == posFinal[1]){
        return;
    }
    ...
    //cout << "Casillero: " << casillero->getPos()[0] << " , " << casillero->getPos()[1] << endl;
    ...
    *costoEnergia += peso;
    ...
    //cout << "Costo Energia: " << *costoEnergia << endl;
